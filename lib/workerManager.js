...
class WorkerManager {
  ...
  async start(workerId, { noResume = false, startedBy = "panel" } = {}) {
    ...
    const nodeOptions =
      (def.env && def.env.NODE_OPTIONS) || process.env.NODE_OPTIONS || "--max-old-space-size=768";
    let child;
    try {
    ...
    child.on("exit", async (code, signal) => {
      ...
      let status = "error";
      let summary = `خروج با کد ${code}`;
      if (signal === "SIGTERM") {
        status = "stopped";
        summary = "توسط پنل متوقف شد";
      } else if (signal === "SIGKILL") {
        status = "error";
        summary = "فرایند کشته شد (احتمالاً کمبود حافظه / OOM)";
      } else if (code === 0) {
        status = "success";
        summary = progress.extra || "با خروج موفق تمام شد";
      }
      await this.runs.updateOne(
        { _id: runId },
        {
